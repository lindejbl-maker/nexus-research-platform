...
const gemini = {
  ...
  async generateHypotheses(topic, field = '', count = 3, profileContext = '') {
    ...
    const system = `You are a rigorous molecular biologist and specialized domain expert reviewing this hypothesis for publication. You identify untested research gaps, but you must flag loose terminology, pathway conflations, and unsupported jumps. You always respond with valid JSON only — no markdown fences, no preamble, no explanation.`;
    const prompt = `${profileContext}${ragContext}Research topic: "${topic}"\n${field ? `Research field: ${field}` : ''}\n\nAnalyse the real papers in the CURRENT LITERATURE CONTEXT above (if provided), then identify ${count} genuinely novel, testable hypotheses representing gaps that NONE of those papers have addressed. Tailor hypotheses to the researcher context if provided. You must be strictly grounded.\n\nReturn a JSON array where each item has exactly these fields:\n- "id": number (1-based)\n- "title": string (max 10 words, catchy and specific)\n- "hypothesis": string (a clear 2-3 sentence testable statement)\n- "mechanism_details": string (The specific protein/pathway involved, why that mechanism — not adjacent ones — is the target, and what distinguishes it from existing approaches)\n- "rationale": string (2-3 sentences explaining why this gap exists in the current literature)\n- "expert_critique": string (Your brutal, honest critique of this hypothesis. Review mechanism accuracy, identify any mechanistic inaccuracies, conflations between related but distinct pathways, or overclaims. Flag anything a domain expert would challenge. Leave empty if flawless.)\n- "confidence_tier": string (exactly one of: "✅ Established in literature", "⚠️ Inferred / extrapolated", "🔬 Speculative / frontier")\n- "novelty_score": number (integer 70–99)\n- "gap_type": string (one of: "untested combination", "missing population", "unexplored mechanism", "methodology gap", "cross-disciplinary gap")\n- "experiment_hint": string (one concrete sentence on how to test this)\n- "grounded": boolean (true if you can point to specific papers above that confirm this gap exists)\n- "is_assumption": boolean (true if the hypothesis rests on an untested STARTING ASSUMPTION)\n- "evidence_count": number (your best estimate of how many papers in the literature address this specific claim area; use 0 if truly novel)\n\nReturn ONLY the JSON array.`;
